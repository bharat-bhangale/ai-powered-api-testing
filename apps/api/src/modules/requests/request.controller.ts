...
export async function createRequest(req: Request, res: Response): Promise<void> {
  try {
    const { name, collectionId, folderId, method, url, headers, params, body, auth } = req.body;
    if (!name || !collectionId || !method) {
      res.status(400).json({
        success: false,
        error: { code: 'VALIDATION_ERROR', message: 'Name, collectionId, and method are required' },
      });
      return;
    }
    const request = await requestService.create(req.userId!, {
      name, collectionId, folderId, method, url, headers, params, body, auth,
    });
...
export async function updateRequest(req: Request, res: Response): Promise<void> {
  try {
    const id = req.params.id as string;
    const request = await requestService.update(req.userId!, id, req.body);
    res.json({ success: true, data: { request } });
  } catch (error: unknown) {
